...
undo/redo, grapheme-safe maxLength enforcement, composition protection, and safe
disabled/read-only/reset behavior. VoiceTextarea and VoiceInput now notify one
onValueChange callback for typing and voice edits, including undo/redo;
uncontrolled React onChange also receives dictation. Recording configuration is
sampled at Start so inline provider factories do not interrupt recording.
...
defaults to gpt-transcribe with 500 ms server VAD: the live model remains opt-in
for one manual-commit segment per recording.
